...
import static org.dragon.sandbox.SandboxConstants.*;

/**
 * 沙箱配置解析器
 * 负责合并 Agent 级别和全局级别的沙箱配置
 */
public final class SandboxConfigResolver {
    ...
    /**
     * 解析沙箱作用域
     * 优先使用显式指定的 scope，否则向后兼容 perSession 参数
     *
     * @param scope 显式指定的作用域
     * @param perSession 兼容旧的按会话隔离标志
     * @return 沙箱作用域
     */
    ...
    /**
     * 解析 Docker 配置
     * 合并 Agent 级别和全局级别的 Docker 设置，Agent 配置优先
     */
