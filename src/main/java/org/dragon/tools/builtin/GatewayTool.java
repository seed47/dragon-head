...
/**
 * 网关管理工具 — 重启、配置操作。
 * 对应 TypeScript 的 tools/gateway-tool.ts。
 *
 * <p>
 * 支持的操作：
 * </p>
 * <ul>
 * <li>{@code restart} — 计划网关重启（SIGUSR1）</li>
 * <li>{@code config.get} — 获取当前网关配置</li>
 * <li>{@code config.apply} — 替换整个配置</li>
 * <li>{@code config.patch} — 合并部分配置</li>
 * <li>{@code update.run} — 触发网关更新</li>
 * </ul>
 */
@Slf4j
public class GatewayTool implements AgentTool {
    ...
    private GatewayTool(String agentSessionKey, ConfigProperties config) {
        this.agentSessionKey = agentSessionKey;
        this.config = config;
    }

    public static GatewayTool create(String agentSessionKey, ConfigProperties config) {
