...
/**
 * Agent 列表工具 — 列出可用于子 Agent 生成的 Agent ID。
 * 对应 TypeScript 的 tools/agents-list-tool.ts。
 */
@Slf4j
public class AgentsListTool implements AgentTool {
    ...
    private ToolResult doExecute(ToolContext context) {
        try {
            ConfigProperties config = context.getConfig();
            List<AgentInfo> agents = new ArrayList<>();
            agents.add(new AgentInfo("main", null));

            // ConfigProperties doesn't have agents config
